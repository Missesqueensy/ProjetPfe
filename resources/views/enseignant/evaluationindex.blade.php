...
<head>
    ...
    <meta property="og:description" content="Plateforme de gestion des évaluations pour enseignants">
    <style>
        /* Ajout de styles pour la liste déroulante */
        .evaluations-list {
            max-height: 300px;
            overflow-y: auto;
            margin-bottom: 10px;
        ...
        .evaluations-toggle:hover {
            background-color: rgba(0,0,0,0.05);
        }
        .evaluations-toggle .las {
            transition: transform 0.3s;
        }
        .evaluations-toggle.collapsed .las {
            transform: rotate(-90deg);
        }
        .evaluations-items {
            padding-left: 15px;
        }
        .evaluations-items.collapse:not(.show) {
            display: none;
        }
    </style>
</head>
...
<div class="dashboard-container">
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            <ul>
                ...
                <!-- Liste déroulante des évaluations -->
                @isset($evaluations)
                <li class="evaluations-list">
                    <div class="evaluations-toggle collapsed" data-bs-toggle="collapse" data-bs-target="#evaluationsCollapse">
                        <span>
                            <span class="las la-angle-down"></span>
                            Mes Évaluations
                        </span>
                        <span class="badge bg-primary rounded-pill">{{ count($evaluations) }}</span>
                    </div>
                    <div class="evaluations-items collapse" id="evaluationsCollapse">
                        @foreach($evaluations as $evaluation)
                            <a href="{{ route('enseignant.notes.index', ['evaluation' => $evaluation->id_evaluation]) }}" class="d-block py-2 px-3">
                                <span class="la la-check-circle"></span>
                                {{ $evaluation->titre }}
                            </a>
                        @endforeach
    ...
    <!-- Le reste de votre code main-content reste inchangé -->
    <div class="main-content">
        <header class="sticky-top bg-white shadow-sm">
            <div class="container-fluid py-2">
                <div class="d-flex justify-content-between align-items-center">
...
<script>
    ...
    // Animation pour le toggle des évaluations
    document.querySelectorAll('.evaluations-toggle').forEach(toggle => {
